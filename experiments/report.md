...
### 2.4 Pass/Fail 阈值（基于实验数据校准）

| Benchmark | 初始阈值 | 校准后阈值 | 校准依据 |
|---|---|---|---|
| B1 | ≥ 0.6 | ≥ 0.63 | zero_shot with_skill=0.636，few_shot=0.677，中位数约 0.65，取保守值 |
| B2 | ≥ 0.5 | ≥ 0.68 | 三场景 with_skill 均在 0.68-0.73 之间，取下界 |
| B3 | ≥ 0.5 | 维持 ≥ 0.5 | zero_shot Gain 为负，few_shot 数据待补充，暂不校准 |

**校准说明**：基于本次实验 2 个 Skill 版本（zero_shot + few_shot）共 15 个 with_skill 数据点校准。样本量偏小（理想值 ≥ 20），后续扩充实验后需重新校准。B3 因 few_shot 数据缺失，暂维持先验阈值。
...
**结论**：Rule 断言适合格式合规性检查，不适合存在性检查。基于人工复核，zero_shot B1 实际质量约 0.75，当前自动评估 0.636 因 Rule 断言误判偏低约 0.10-0.15。
...
从技术层面，本框架实现了端到端的自动评估：输入一个 SKILL.md，自动生成 PRD，自动执行断言，自动输出 Gain 和 Tier，全程无需人工干预。

### 6.2 但评估对象发生了主动转移

题目要求"评估 Agent 生成该目标产物的能力"，最直接的解读是给 PRD 打绝对分数。我们的实际做法是评估 **Skill 对 Agent 输出的增量影响**，而非直接评估 Agent 能力本身。

这是一个主动的设计选择：绝对分数无法区分"Skill 本身好"和"模型基础能力强"，Skill Gain 才能回答"这个 Skill 值得用吗"这个更有实际意义的工程问题。

### 6.3 自动评估尚未解决的三个核心问题

**问题一：断言覆盖率未经系统验证**

我们手工设计了 22 条 B1 断言，但这 22 条能覆盖 PRD 质量的多少比例？没有经过系统性验证。断言的设计本身存在主观性，不同的产品经理可能设计出完全不同的断言集合。

**问题二：没有黄金标准集校准评估准确率**

Rule 断言的系统性低估问题，是通过人工随机抽查发现的，不是系统性发现的。我们没有一个人工标注的黄金标准集来验证自动评估的准确率，无法量化"我们的评估有多准"。

**问题三：Judge 本身没有被评估**

Claude 作为 Judge 的判断准确率是多少？我们假设 Claude 是一个可靠的评估者，但没有对 Judge 本身做任何评估。这是一个悬而未决的元评估问题。

### 6.4 一句话总结

我们探索并实现了一种自动评估方式，它在工程上可运行、在逻辑上自洽、在发现上有价值。但它仍然是一个原型，而非经过严格验证的评估系统。主动承认这一点，是保证实验结论可信度的前提。
...
| 2026-04-01 | 新增 v2 断言版本，B1/B2 few_shot 实验完成，Pass/Fail 阈值基于实验数据校准 |
| 待补充 | B3 few_shot 实验结果 |
| 待补充 | v2 断言版本完整实验结果 |
